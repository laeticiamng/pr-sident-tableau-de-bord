export const platformsTranslations = {
  fr: {
    hero: {
      badge: "Écosystème Complet",
      title: "Nos",
      titleAccent: "Plateformes",
      subtitle: "Sept solutions innovantes, une vision unifiée. Chaque plateforme excelle dans son domaine.",
    },
    stats: {
      platforms: "Plateformes",
      evolutions: "Évolutions",
      tests: "Tests",
      structures: "Structures",
      integrations: "Intégrations",
    },
    filter: {
      all: "Toutes",
      production: "Production",
      prototype: "Prototype",
    },
    status: {
      production: "Production",
      prototype: "Prototype",
      development: "Développement",
    },
    labels: {
      modules: "modules",
      visitSite: "Visiter le site",
      evolutions: "Évolutions",
      structures: "Structures",
      tests: "Tests",
      versions: "Versions",
      integrations: "Intégrations",
      lastUpdate: "Dernière MAJ",
    },
    governance: {
      badge: "Gouvernance Unifiée",
      title1: "Un siège.",
      title2: "Sept excellences.",
      subtitle: "Toutes nos plateformes partagent la même infrastructure backend, les mêmes standards de sécurité et la même rigueur d'exécution.",
      cta: "Accéder au tableau de bord",
    },
  },
  en: {
    hero: {
      badge: "Complete Ecosystem",
      title: "Our",
      titleAccent: "Platforms",
      subtitle: "Seven innovative solutions, one unified vision. Each platform excels in its domain.",
    },
    stats: {
      platforms: "Platforms",
      evolutions: "Evolutions",
      tests: "Tests",
      structures: "Structures",
      integrations: "Integrations",
    },
    filter: {
      all: "All",
      production: "Production",
      prototype: "Prototype",
    },
    status: {
      production: "Production",
      prototype: "Prototype",
      development: "Development",
    },
    labels: {
      modules: "modules",
      visitSite: "Visit website",
      evolutions: "Evolutions",
      structures: "Structures",
      tests: "Tests",
      versions: "Versions",
      integrations: "Integrations",
      lastUpdate: "Last update",
    },
    governance: {
      badge: "Unified Governance",
      title1: "One headquarters.",
      title2: "Seven excellences.",
      subtitle: "All our platforms share the same backend infrastructure, the same security standards and the same execution rigor.",
      cta: "Access the dashboard",
    },
  },
  de: {
    hero: {
      badge: "Vollständiges Ökosystem",
      title: "Unsere",
      titleAccent: "Plattformen",
      subtitle: "Sieben innovative Lösungen, eine einheitliche Vision. Jede Plattform glänzt in ihrem Bereich.",
    },
    stats: {
      platforms: "Plattformen",
      evolutions: "Entwicklungen",
      tests: "Tests",
      structures: "Strukturen",
      integrations: "Integrationen",
    },
    filter: {
      all: "Alle",
      production: "Produktion",
      prototype: "Prototyp",
    },
    status: {
      production: "Produktion",
      prototype: "Prototyp",
      development: "Entwicklung",
    },
    labels: {
      modules: "Module",
      visitSite: "Webseite besuchen",
      evolutions: "Entwicklungen",
      structures: "Strukturen",
      tests: "Tests",
      versions: "Versionen",
      integrations: "Integrationen",
      lastUpdate: "Letztes Update",
    },
    governance: {
      badge: "Einheitliche Governance",
      title1: "Ein Hauptsitz.",
      title2: "Sieben Spitzenleistungen.",
      subtitle: "Alle unsere Plattformen teilen die gleiche Backend-Infrastruktur, die gleichen Sicherheitsstandards und die gleiche Ausführungsstrenge.",
      cta: "Zum Dashboard",
    },
  },
} as const;
