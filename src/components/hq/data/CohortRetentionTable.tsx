...
interface CohortData {
  month: string;
  users: number;
  retention: number[]; // Pourcentages de rétention M0, M1, M2, etc.
}
...
const DEFAULT_COHORT_DATA: CohortData[] = [
  { month: "Oct 2025", users: 145, retention: [100, 88, 75, 68, 62] },
  { month: "Nov 2025", users: 168, retention: [100, 82, 70, 65] },
  { month: "Déc 2025", users: 195, retention: [100, 86, 73] },
  { month: "Jan 2026", users: 220, retention: [100, 84] },
  { month: "Fév 2026", users: 245, retention: [100] },
];

export function CohortRetentionTable({ data = DEFAULT_COHORT_DATA, className }: CohortRetentionTableProps) {
  ...
  return (
    <Card className={cn("card-executive", className)}>
      <CardHeader>
        ...
        <CardDescription>
          Rétention moyenne M1 : {avgRetention}%
        </CardDescription>
      </CardHeader>
      <CardContent>
              ...
              {data.map(cohort => (
                <tr key={cohort.month} className="border-b last:border-0 hover:bg-muted/30">
                  ...
                  <td className="text-center py-3 px-2">
                    <Badge variant="subtle">{cohort.users}</Badge>
                  </td>
