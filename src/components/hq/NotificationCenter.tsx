...
export function NotificationCenter({ className }: NotificationCenterProps) {
  ...
  const getNotificationIcon = (type: RealtimeNotification["type"]) => {
    switch (type) {
      case "approval_required": return <Bell className="h-4 w-4 text-warning" />;
      case "approval_resolved": return <CheckCircle className="h-4 w-4 text-success" />;
      case "alert": return <AlertTriangle className="h-4 w-4 text-destructive" />;
      case "run_completed": return <Sparkles className="h-4 w-4 text-accent" />;
      case "contact_message": return <Mail className="h-4 w-4 text-primary" />;
      ...
      <PopoverContent className="w-[calc(100vw-2rem)] sm:w-80 max-w-sm p-0" align="end">
        <div className="flex items-center justify-between p-3 border-b">
          ...
          <div className="flex items-center gap-1">
            {unreadCount > 0 && (
              <Button variant="ghost" size="icon" className="h-7 w-7" onClick={markAllAsRead} title={t.markAllAsRead}>
                <CheckCheck className="h-4 w-4" />
              </Button>
            )}
            {notifications.length > 0 && (
              <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={clearNotifications} title={t.clearAll}>
          ...
          {notifications.length === 0 ? (
            <div className="p-6 text-center text-muted-foreground">
              ...
              <p className="text-xs mt-1">
                {isConnected ? t.realtimeActive : t.reconnecting}
              </p>
            </div>
          ) : (
            <div className="divide-y">
              {notifications.map((notification) => (
                <div
                  key={notification.id}
                  className={cn("p-3 hover:bg-muted/50 transition-colors cursor-pointer", !notification.read && "bg-primary/5")}
                  onClick={() => markAsRead(notification.id)}
                >
                  <div className="flex gap-3">
                    <div className={cn("flex-shrink-0 p-2 rounded-lg border", getUrgencyColor(notification.urgency))}>
                      {getNotificationIcon(notification.type)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <p className="font-medium text-sm truncate">{notification.title}</p>
                        {!notification.read && <Circle className="h-2 w-2 fill-primary text-primary flex-shrink-0 mt-1" />}
                      </div>
                      <p className="text-xs text-muted-foreground line-clamp-2 mt-0.5">{notification.message}</p>
                      <p className="text-xs text-muted-foreground/70 mt-1">
                        {formatDistanceToNow(notification.timestamp, { addSuffix: true, locale })}
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <div className="p-2 border-t">
          <p className="text-xs text-center text-muted-foreground">
            <Circle className={cn("inline h-2 w-2 mr-1 fill-current", isConnected ? "text-success" : "text-destructive")} />
