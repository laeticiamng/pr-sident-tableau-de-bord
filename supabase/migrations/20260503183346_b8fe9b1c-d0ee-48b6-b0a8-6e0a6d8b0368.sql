...
CREATE INDEX IF NOT EXISTS studio_calls_status_idx ON hq.studio_calls (status, created_at DESC);
CREATE INDEX IF NOT EXISTS studio_calls_deadline_idx ON hq.studio_calls (deadline);
...
CREATE TABLE IF NOT EXISTS hq.studio_blueprints (
  ...
  status TEXT NOT NULL DEFAULT 'draft' CHECK (status IN ('draft', 'in_review', 'finalized', 'sent', 'archived')),
...
CREATE INDEX IF NOT EXISTS studio_blueprints_status_idx ON hq.studio_blueprints (status, created_at DESC);
CREATE INDEX IF NOT EXISTS studio_blueprints_opportunity_idx ON hq.studio_blueprints (opportunity_id);
CREATE INDEX IF NOT EXISTS studio_blueprints_call_idx ON hq.studio_blueprints (call_id);
...
CREATE TABLE IF NOT EXISTS hq.studio_deals (
  ...
  next_action TEXT,
  status TEXT NOT NULL DEFAULT 'draft' CHECK (status IN ('draft', 'proposed', 'in_negotiation', 'accepted', 'signed', 'rejected', 'archived')),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT studio_deals_project_name_chk CHECK (char_length(project_name) BETWEEN 1 AND 280),
  CONSTRAINT studio_deals_equity_min_chk CHECK (equity_percentage_min IS NULL OR (equity_percentage_min >= 0 AND equity_percentage_min <= 100)),
  CONSTRAINT studio_deals_equity_max_chk CHECK (equity_percentage_max IS NULL OR (equity_percentage_max >= 0 AND equity_percentage_max <= 100))
);

CREATE INDEX IF NOT EXISTS studio_deals_status_idx ON hq.studio_deals (status, created_at DESC);
CREATE INDEX IF NOT EXISTS studio_deals_blueprint_idx ON hq.studio_deals (blueprint_id);
...
CREATE INDEX IF NOT EXISTS studio_advisory_status_idx ON hq.studio_advisory (status, created_at DESC);
...
CREATE INDEX IF NOT EXISTS studio_documents_type_idx ON hq.studio_documents (document_type, created_at DESC);

-- Triggers updated_at
CREATE TRIGGER update_studio_opportunities_updated_at BEFORE UPDATE ON hq.studio_opportunities FOR EACH ROW EXECUTE FUNCTION hq.update_updated_at();
CREATE TRIGGER update_studio_calls_updated_at BEFORE UPDATE ON hq.studio_calls FOR EACH ROW EXECUTE FUNCTION hq.update_updated_at();
CREATE TRIGGER update_studio_blueprints_updated_at BEFORE UPDATE ON hq.studio_blueprints FOR EACH ROW EXECUTE FUNCTION hq.update_updated_at();
CREATE TRIGGER update_studio_deals_updated_at BEFORE UPDATE ON hq.studio_deals FOR EACH ROW EXECUTE FUNCTION hq.update_updated_at();
CREATE TRIGGER update_studio_advisory_updated_at BEFORE UPDATE ON hq.studio_advisory FOR EACH ROW EXECUTE FUNCTION hq.update_updated_at();
CREATE TRIGGER update_studio_documents_updated_at BEFORE UPDATE ON hq.studio_documents FOR EACH ROW EXECUTE FUNCTION hq.update_updated_at();
...
CREATE POLICY "Owner accès complet" ON hq.studio_opportunities FOR ALL TO authenticated USING (public.is_owner()) WITH CHECK (public.is_owner());
CREATE POLICY "Owner accès complet" ON hq.studio_calls         FOR ALL TO authenticated USING (public.is_owner()) WITH CHECK (public.is_owner());
CREATE POLICY "Owner accès complet" ON hq.studio_blueprints    FOR ALL TO authenticated USING (public.is_owner()) WITH CHECK (public.is_owner());
CREATE POLICY "Owner accès complet" ON hq.studio_deals         FOR ALL TO authenticated USING (public.is_owner()) WITH CHECK (public.is_owner());
CREATE POLICY "Owner accès complet" ON hq.studio_advisory      FOR ALL TO authenticated USING (public.is_owner()) WITH CHECK (public.is_owner());
CREATE POLICY "Owner accès complet" ON hq.studio_documents     FOR ALL TO authenticated USING (public.is_owner()) WITH CHECK (public.is_owner());

-- RPC functions
CREATE OR REPLACE FUNCTION public.list_studio_opportunities()
RETURNS SETOF hq.studio_opportunities LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, hq
AS $$ BEGIN
...
CREATE OR REPLACE FUNCTION public.create_studio_opportunity(
  p_title TEXT, p_domain TEXT DEFAULT NULL, p_source_type TEXT DEFAULT NULL,
  p_source_url TEXT DEFAULT NULL, p_description TEXT DEFAULT NULL, p_problem_statement TEXT DEFAULT NULL
) RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, hq
AS $$ DECLARE new_id UUID;
BEGIN
  IF NOT public.is_owner() THEN RAISE EXCEPTION 'Accès non autorisé'; END IF;
  INSERT INTO hq.studio_opportunities (title, domain, source_type, source_url, description, problem_statement, created_by)
  VALUES (p_title, p_domain, p_source_type, p_source_url, p_description, p_problem_statement, auth.uid())
...
RETURNS SETOF hq.studio_calls LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, hq
AS $$ BEGIN
  IF NOT public.is_owner() THEN RAISE EXCEPTION 'Accès non autorisé'; END IF;
...
CREATE OR REPLACE FUNCTION public.create_studio_call(
  p_title TEXT, p_call_type TEXT, p_issuer TEXT DEFAULT NULL, p_source_url TEXT DEFAULT NULL,
  p_deadline DATE DEFAULT NULL, p_domain TEXT DEFAULT NULL, p_eligibility TEXT DEFAULT NULL,
  p_estimated_budget TEXT DEFAULT NULL
) RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, hq
AS $$ DECLARE new_id UUID;
BEGIN
  IF NOT public.is_owner() THEN RAISE EXCEPTION 'Accès non autorisé'; END IF;
  INSERT INTO hq.studio_calls (title, call_type, issuer, source_url, deadline, domain, eligibility, estimated_budget)
  VALUES (p_title, p_call_type, p_issuer, p_source_url, p_deadline, p_domain, p_eligibility, p_estimated_budget)
  RETURNING id INTO new_id;
  INSERT INTO hq.audit_logs (actor_type, actor_id, action, resource_type, resource_id, details)
...
RETURNS SETOF hq.studio_blueprints LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, hq
AS $$ BEGIN
  IF NOT public.is_owner() THEN RAISE EXCEPTION 'Accès non autorisé'; END IF;
...
RETURNS SETOF hq.studio_deals LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, hq
AS $$ BEGIN
  IF NOT public.is_owner() THEN RAISE EXCEPTION 'Accès non autorisé'; END IF;
...
RETURNS SETOF hq.studio_advisory LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, hq
AS $$ BEGIN
  IF NOT public.is_owner() THEN RAISE EXCEPTION 'Accès non autorisé'; END IF;
...
RETURNS SETOF hq.studio_documents LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, hq
AS $$ BEGIN
  IF NOT public.is_owner() THEN RAISE EXCEPTION 'Accès non autorisé'; END IF;
...
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, hq
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'calls_with_deadline_30d', (SELECT COUNT(*) FROM hq.studio_calls WHERE deadline IS NOT NULL AND deadline >= CURRENT_DATE AND deadline <= CURRENT_DATE + INTERVAL '30 days' AND status NOT IN ('lost', 'recycle', 'not_relevant')),
